...
function setHash(redisClient, fields, cb) {
    const cmds = fields.map(field => {
        ...
        const value = JSON.stringify(getKafkaEntry(bucket, key, site));
...
describe('Backbeat Server', () => {
    ...
    describe('metrics routes', () => {
        ...
        describe('Retry feature with Redis', () => {
            ...
            const testVersionId =
                '393834373736353330393839333639393939393952473030312020323436';
                ...
                async.timesLimit(500, 10, (i, next) => {
                    ...
                    makeRetryPOSTRequest(body, (err, res) => {
                        ...
                        getResponseBody(res, (err, resBody) => {
                            assert.ifError(err);
                            const body = JSON.parse(resBody);
                            assert.strictEqual(body.length, 500 * 5);
                            done();
                        });
                    });
                });
