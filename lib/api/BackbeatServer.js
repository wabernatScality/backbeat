...
class BackbeatServer {
     ...
     * @param {object} data - if error, can carry a specific error message
     * @param {object} req - request object
     * @param {object} res - response object
     * @return {undefined}
     */
    _logRequestEnd(logger, data, req, res) {
        const info = {
    ...
    _requestListener(req, res) {
        ...
        && this._areConditionsOk(bbRequest)) {
            ...
            let routeDetails = routes.find(r => r.path === route);
            if (!routeDetails) {
                // a valid route with an optional param
                const subRoute = route.substr(0, route.lastIndexOf('/'));
                routeDetails = routes.find(r => r.path === subRoute);

                const site = route.substr(route.lastIndexOf('/') + 1);
                routeDetails.site = site;
            } else {
                routeDetails.site = '*';
            }
    ...
    _response(data, backbeatRequest) {
        ...
        this._logRequestEnd(log, data, req, res);
