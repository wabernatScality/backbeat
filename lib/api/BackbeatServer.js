        ...
        && this._areConditionsOk(bbRequest)) {
            ...
            */
            ...
            if (bbRequest.getRoute() === 'healthcheck') {
                ...
                if (rDetails.status && rDetails.status === 'failed') {
                    const { bucket, key, versionId } = rDetails;
                    if (bucket && key && versionId) {
                        routeDetails = routes.find(r =>
                            r.method === 'getFailedCRR');
                        routeDetails = Object.assign({}, routeDetails,
                            rDetails);
                    } else {
                        routeDetails = routes.find(r => {
