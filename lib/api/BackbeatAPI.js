...
class BackbeatAPI {
    ...
    isValidRoute(bbRequest) {
        ...
        */
        ...
        const extensions = routes.reduce((store, r) => {
            if (rDetails.extension === 'crr' && r.status === 'failed') {
        ...
        const validRoutes = [];
        routes.forEach(r => {
            if (r.status && r.status === rDetails.status) {
                return validRoutes.push(r);
            }
            if (!r.extensions) {
                return;
            }
    ...
    getFailedCRR(details, cb) {
        ...
        const cmds = ['hscan', CRR_FAILED_HASH_KEY, 0, 'MATCH', pattern];
    ...
    getAllFailedCRR(details, cb) {
        const cmds = ['hgetall', CRR_FAILED_HASH_KEY];
        this._redisClient.batch([cmds], (err, res) => {
            if (err) {
                return cb(err);
            }
    ...
    _parseRetryFailedCRR(body) {
        ...
        try {
            ...
            return {
                err: errors.MalformedPOSTRequest.customizeDescription(msg),
            };
        }
        if (!Array.isArray(reqBody) || reqBody.length === 0) {
            return {
                err: errors.MalformedPOSTRequest.customizeDescription(
        ...
        if (errMsg) {
            return {
                err: errors.MalformedPOSTRequest.customizeDescription(errMsg),
            };
        }
        return { reqBody };
    }
    ...
    retryFailedCRR(details, body, cb) {
        const { err, reqBody } = this._parseRetryFailedCRR(body);
        if (err) {
            return cb(err);
        }
        ...
        const cmds = ['hmget', CRR_FAILED_HASH_KEY, ...fields];
        this._redisClient.batch([cmds], (err, res) => {
            if (err) {
                return cb(err);
            }
