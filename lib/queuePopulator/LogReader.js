...
class LogReader {
    ...
    _setEntryBatch(entryBatch) {
        this.log.info('LogReader._setEntryBatch');
        this._extensions.forEach(ext => ext.setBatch(entryBatch));
    }

    _endEntryBatch() {
        this.log.info('LogReader._endEntryBatch');
    ...
    _processPrepareEntries(batchState, done) {
        ...
        this.log.info('LogReader._processPrepareEntries initial batchState:', {
            batchState,
        });
        ...
        logRes.log.on('data', record => {
            this.log.info('LogReader._processPrepareEntries DATA EVENT');
            this.log.info('LogReader._processPrepareEntries data batchState:', {
                batchState,
            });
            logStats.nbLogRecordsRead += 1;
            // This event is occurring before _setEntryBatch?
        ...
        logRes.log.on('error', err => {
            this.log.info('LogReader._processPrepareEntries ERROR EVENT');
        ...
        logRes.log.on('end', () => {
            this.log.info('LogReader._processPrepareEntries END EVENT');
            this.log.debug('ending record stream');
            this._endEntryBatch();
            this.log.info('LogReader._processPrepareEntries end batchState:', {
                batchState,
            });
            return done();
        });
